...
final class IconResolver
{
    ...
    private function skillSprites(): array
    {
        return $this->skillSprites ??= $this->remembered('skillSprites', function (): array {
            ...
            foreach ($this->loadJson('public/tree/current/assets/skills.json')['frames'] ?? [] as $key => $entry) {
            ...
            [$sheetW, $sheetH] = $this->webpSize('public/tree/current/assets/skills.webp');

            return ['frames' => $frames, 'sheetW' => $sheetW, 'sheetH' => $sheetH];
        });
    }

    /**
     * Pixel dimensions of a WebP file from its header (VP8X / VP8L / lossy VP8),
     * falling back to [0, 0] when the file is missing or unrecognised.
     *
     * @return array{0: int, 1: int}
     */
    private function webpSize(string $relative): array
    {
        ...
        $header = (string) file_get_contents($path, false, null, 0, 30);

        if (strlen($header) < 30 || ! str_starts_with($header, 'RIFF')) {
            return [0, 0];
        }

        $bytes = array_values(unpack('C*', $header) ?: []);
        $format = substr($header, 12, 4);

        // Byte offsets follow the WebP container spec for each VP8 variant.
        if ($format === 'VP8X') {
        ...
        if ($format === 'VP8L') {
            $bits = $bytes[21] | $bytes[22] << 8 | $bytes[23] << 16 | $bytes[24] << 24;
