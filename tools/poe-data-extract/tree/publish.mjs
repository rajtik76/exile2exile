...
// DATA_JSON_ONLY (the pre-test extraction) publishes only the JSON the server reads -
// data.json + the atlas frame maps with their sheet size - and skips the heavy webp
// encode and centre art, which are client-render-only.
const jsonOnly = Boolean(process.env.DATA_JSON_ONLY);
...
for (const { name, quality } of ATLASES) {
  const srcPng = fileURLToPath(new URL(`assets/${name}.png`, OUT));
  const frameMap = JSON.parse(readFileSync(fileURLToPath(new URL(`assets/${name}.json`, OUT)), 'utf8'));
  // Carry the atlas sheet's pixel size in the frame map so the server can crop notable
  // art from it without ever reading the binary webp (which the JSON-only path skips).
  const meta = await sharp(srcPng).metadata();
  frameMap.sheet = { w: meta.width, h: meta.height };
  writeFileSync(fileURLToPath(new URL(`${name}.json`, ASSETS)), JSON.stringify(frameMap));

  if (!jsonOnly) {
    await pngToWebp(srcPng, fileURLToPath(new URL(`${name}.webp`, ASSETS)), quality);
  }
  console.log(`atlas ${name}${jsonOnly ? ' (json only)' : ` -> webp (q${quality})`}`);
}
