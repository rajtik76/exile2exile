...
## 2026-07-21
- Fixed the "Back to top" waypoint button showing on top of the passive tree's fullscreen view, where it makes no sense since there's nothing to scroll to while the tree fills the screen. The page section the tree sits in sets its own stacking context (to layer above the class-portrait backdrop), which trapped the fullscreen stage's z-index inside that section no matter how high it was set - so the button, rendered at the page root, could still end up on top of the "fullscreen" tree. The button is now hidden for as long as the tree stays fullscreen, instead of fighting it for a stacking order that doesn't compare cleanly across that boundary.
- Fixed regular passive tree node tooltips showing their stat/flavour text in what looked like all caps: that text was set in the game's small-caps display face, which is correct for a title but reads as shouting for prose - notable and keystone tooltips already used the regular face for their body text, and plain nodes now do too.
...
- Fixed the passive tree canvas rendering nonstop in the background even while sitting perfectly still - it was redrawing every frame forever instead of only when something actually changed (panning, hovering, a highlight pulsing), burning CPU and battery on an otherwise static screen.
- Build planner phases (Act I-IV, Interlude, Early Endgame) are no longer mandatory or fixed in order: pick only the ones your build needs, add them in whatever order suits your guide, reorder any tab with the new move-left/right arrows, and rename any phase - including the built-in ones, which now serve only as prefilled defaults. The one rule left is that a plan always keeps at least one phase.
- The site header no longer stays pinned while you scroll - it was fighting the build planner's own phase-tab bar for the top of the screen. A guide page can now grow long without either bar crowding the other; a small waypoint-style button appears at the bottom-right once you've scrolled a screen down, to jump straight back to the top.
...
- A hidden category really stays hidden: it is the player's word and beats everything, including live prices - an uncut gem that happens to be worth a lot on the market will not sneak back in through the economy highlights. Only the build-aware overlay (bases and modifiers the build itself wants) still shows on top. (Fixed same day: the exclusion first checked only exact base-type names, so a category hidden by a bare NeverSink rule - like uncut gems, matched by name alone regardless of level - still let its priced variants through, since poe2scout prices them per level, e.g. "Uncut Support Gem (Level 5)". The exclusion now mirrors the game's own two BaseType forms, exact and "contains", so a hidden category is hidden in every form it can appear in.)
